...
|---|---|---|---|---|
| `M8 Compatibility Profile v1` | `Runtime-partial` | The repo has compatibility docs and tests, plus a small early external-app lane, but the implemented ABI surface is still narrow. | A declared compatibility subset runs real external programs on the default lane with stable loader, process, file, and socket behavior. | Broaden the syscall and loader surface, tie it to shipped userspace, and prove the supported subset with real binaries rather than only profile models. |
| `M17 Compatibility Profile v2` | `Evidence-first` | The v2 compatibility lane is mostly a richer contract and gate story. | The supported compatibility profile is wider in the actual runtime, not only in the documents. | Implement more process, loader, signal, fd, and error semantics, and keep the v2 tests grounded in real binary execution. |
| `M27 External App Compatibility Program v3` | `Evidence-first` | Tiering and class-matrix evidence exist, but the repo does not visibly run a large real app set on the default lane. | The declared app tiers launch and behave on the real system with packaging, lifecycle, and failure reporting that users can consume. | Run representative third-party apps on the live OS, support the ABI and userspace services they need, and publish the exact unsupported edges instead of grading them from synthetic suites alone. |
| `M36 Compatibility Surface Expansion v1` | `Evidence-first` | The milestone adds more compatibility contracts and POSIX gap accounting, but not a matching runtime closure. | The OS actually exposes the additional process, file, socket, and readiness surfaces that the profile claims. | Implement the missing syscalls and runtime behavior, keep deferred behavior explicit in code, and distinguish "not supported" from "not yet implemented." |
| `M41 Process + Readiness Compatibility Closure v1` | `Evidence-first` | Fork, clone, epoll, and readiness behavior are mostly described through contracts and negative-path suites. | The runtime either implements these surfaces or rejects them with consistent, documented behavior at the ABI boundary. | Add real process-cloning and readiness semantics where intended, or enforce stable non-support paths directly in the runtime and package manager. |
