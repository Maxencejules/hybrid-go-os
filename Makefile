...
image-blk-badptr: build-blk-badptr
	KERNEL_ELF=kernel-blk-badptr.elf ISO_NAME=os-blk-badptr.iso bash tools/mkimage.sh
...
image-pkg-hash: build-pkg-hash
	python3 tools/mkfs.py $(OUT)/fs-test.img
	KERNEL_ELF=kernel-pkg-hash.elf ISO_NAME=os-pkg-hash.iso bash tools/mkimage.sh
