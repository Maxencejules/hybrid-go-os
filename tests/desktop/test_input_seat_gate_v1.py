"""M49 aggregate gate: input seat wiring and closure checks."""
...
def test_input_seat_gate_v1_wiring_and_artifacts():
    required = [
    ...
    for rel in required:
        assert (ROOT / rel).is_file(), f"missing M49 artifact: {rel}"
    ...
    assert "test-input-seat-v1" in makefile
    for entry in [
    ...
    assert "pytest-input-seat-v1.xml" in makefile
    assert "pytest-hid-event-path-v1.xml" in makefile

    assert "Input seat v1 gate" in ci
    assert "make test-input-seat-v1" in ci
    assert "input-seat-v1-artifacts" in ci
    assert "out/pytest-input-seat-v1.xml" in ci
    ...
    assert runtime_data["contract_id"] == "rugo.seat_input_contract.v1"
    ...
    assert hid_data["runtime_schema"] == "rugo.input_seat_runtime_report.v1"
