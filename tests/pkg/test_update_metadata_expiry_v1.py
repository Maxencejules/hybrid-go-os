"""M26 acceptance: metadata expiry attack remains blocked."""
...
def test_update_metadata_expiry_attack_blocked(tmp_path: Path):
    out = tmp_path / "update-trust-v1.json"
    rc = trust.main(["--out", str(out), "--max-failures", "0"])
    assert rc == 0
    data = json.loads(out.read_text(encoding="utf-8"))
    case = next(c for c in data["cases"] if c["name"] == "expiry_attack")
